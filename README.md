...
캐릭터 대사를 소리로도 들려준다. **브라우저 내장 음성합성(Web Speech API)** 이라 별도 API·비용·키가 없다.

- 헤더의 **`🔇 읽기 OFF` / `🔊 읽기 ON`** 버튼으로 자동 읽기를 켜고 끈다(설정은 이 브라우저에 `retro1990.voice`로 저장).
- 켜 두면 새 답변이 도착할 때마다 한국어(`ko-KR`) 음성으로 자동 재생된다.
- 각 캐릭터 대사 끝의 **`🔊`** 를 누르면 그 대사만 다시 들을 수 있다.
- 음성 미지원 브라우저에선 버튼이 자동으로 숨겨지고, 텍스트 채팅은 그대로 동작한다.
  (한국어 음성 품질·목소리는 브라우저/OS에 설치된 음성에 따라 다르다.)
