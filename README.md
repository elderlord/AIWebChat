...
- **침묵 재촉(idle nudge)**: 답변 후 관람객이 가만히 있으면(13초→22초) 캐릭터가 **먼저 대화를 이어간다**.
  1차는 방금 질문을 다른 말로 다시 건네거나 다음 행동을 안내, 2차는 더 구체적인 힌트/제안(한 턴 최대 2회).
  마이크로 말하려 하거나 입력 중이면 재촉하지 않고, 관람객이 반응하면 카운트가 초기화된다.
